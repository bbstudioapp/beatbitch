...
void main() {
  ...
  group('CoachService — gating voix (preferredGender)', () {
    test(
        'coach avec preferredGender et voiceProbe retournant false → lockedNoVoice',
        () async {
      // Probe : aucune voix male dispo (simule Linux / Windows / Android
      // sans voix mâle locale installée).
      final s = CoachService(voiceProbe: (_) => false);
      await s.load();
      await s.syncFromTotalSeconds(10800); // tier 4 — Marc (tier 3) débloqué

      final marc = s.coaches.firstWhere((c) => c.id == 'coach_07_marc');
      final status = s.evaluate(
        marc,
        playerTotalSeconds: 10800,
        handsEnabled: true,
      );
      expect(status, CoachSelectionStatus.lockedNoVoice);
    });

    test('coach avec preferredGender et probe retournant true → sélectionnable',
        () async {
      final s = CoachService(voiceProbe: (gender) => gender == 'male');
      await s.load();
      await s.syncFromTotalSeconds(10800);
    ...
    test('coach sans preferredGender ignore le probe', () async {
      // Probe qui retournerait false partout : sans préférence de genre,
      // un coach ne doit JAMAIS être lockedNoVoice.
      final s = CoachService(voiceProbe: (_) => false);
      await s.load();
      await s.syncFromTotalSeconds(3600);

      final helene = s.coaches.firstWhere((c) => c.id == 'coach_02_helene');
      final status = s.evaluate(
        helene,
        playerTotalSeconds: 3600,
        handsEnabled: true,
      );
      expect(status, isNot(CoachSelectionStatus.lockedNoVoice));
    });

    test('lockedTier prime sur lockedNoVoice', () async {
      // Marc est encore verrouillé par le tier (joueuse à 0 s) : on doit
      // renvoyer lockedTier avant même d'évaluer la voix.
      final s = CoachService(voiceProbe: (_) => false);
      await s.load();
    ...
    test('setVoiceProbe(null) restaure le comportement sans gating voix',
        () async {
      final s = CoachService(voiceProbe: (_) => false);
      await s.load();
      await s.syncFromTotalSeconds(10800);
      ...
      final marc = s.coaches.firstWhere((c) => c.id == 'coach_07_marc');
      expect(
        s.evaluate(marc, playerTotalSeconds: 10800, handsEnabled: true),
        CoachSelectionStatus.lockedNoVoice,
      );

      s.setVoiceProbe(null);
      expect(
        s.evaluate(marc, playerTotalSeconds: 10800, handsEnabled: true),
        anyOf(
          CoachSelectionStatus.selectedAdvancing,
