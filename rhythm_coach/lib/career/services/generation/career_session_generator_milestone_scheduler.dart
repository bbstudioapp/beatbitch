// Fichier part de `career_session_generator.dart` — orchestration
// de l'insertion des milestones **body** dans la séance.
//
// `generate()` reçoit jusqu'à 2 milestones body (`insertedBodies`) +
// éventuellement une milestone finale (`finalMilestone`, gérée à part).
// Chaque body milestone vise une **fenêtre** `[minInsert, maxInsert]`
// avec une `targetTime` cible (par défaut 30 % / 65 % de la durée
// pour la 1ʳᵉ / 2ᵉ). Le générateur ouvre l'insertion dès que `time`
// atteint la target, ou en urgence dès que `time >= maxInsert` (pour
// ne pas la louper si la chauffe traîne).
//
...
///
/// Mute l'état du générateur via `_pushMilestoneSequence` + injection
/// des unlocks dans `_state.unlockedKeys`. Tient aussi le bookkeeping
/// `bodyStartTime` / `bodyDurationSeconds` consommé par `_assembleResult`.
class _MilestoneScheduler {
  ...
  _MilestoneScheduler._(this._gen, this._pending);
  ...
  factory _MilestoneScheduler.fromBodies(
    ...
    return _MilestoneScheduler._(gen, pending);
  }
  ...
  ({int time, double stamina}) _insertAt(
    ...
    final result = _gen._pushMilestoneSequence(
      ctx,
