...
### Modifié
- **Hand obligatoire piloté par la milestone, plus par le coach** — l'ancien flag `Coach.requirements.requiresHands` (qui bloquait Jade tier 3 si le toggle hand était off, et qui était bypassé par `FreeTrainingBanner.onSwitchToPrincipal`) est retiré. La source de vérité devient `LevelMilestone.requiresHands` (déjà câblée dans `career_screen._start:287`) : si une milestone insérée dans la prochaine session implique biffle/hand, le toggle est forcé à true. Le coach ne dicte plus rien. `CoachSelectionStatus.blockedRequiresHands` supprimé, `coachService.evaluate(handsEnabled:)` ignore désormais le paramètre (gardé pour rétrocompat des call sites). `FreeTrainingBanner.onSwitchToPrincipal` passe désormais par `coachService.evaluate` pour respecter `lockedTier` / `minPlayerSeconds`. Clés ARB `coachErrorRequiresHands` et `coachRequiresHands` retirées des 3 locales. Le champ JSON `requirements.requiresHands` reste accepté silencieusement par `CoachRequirement.fromJson` pour les overrides legacy mais est ignoré.
- **Refonte du système d'unlocks — capability-only** — un unlock est désormais binaire (« autorise l'action »), la durée/BPM tenable est entièrement bornée par le `CapabilityProfile` (axes `hold.<pos>.streak`, `biffle.bpm_max`, `motion_streak.comfort`, etc.), jamais par un palier d'unlock. Disparition des unlocks scalaires `throat_hold_long` / `full_hold_long` / `biffle_fast` / `rhythm_extreme` / `rhythm_head_mid_sustained`. Renommage de `throat_hold_short` → `throat_hold`, `full_hold_short` → `full_hold`, `hold_mid_short` → `hold_mid`. Milestones correspondantes renommées (`intro_hold_throat_short` → `intro_hold_throat`, `intro_hold_full_short` → `intro_hold_full`) et leur séquence pédagogique raccourcie à 6 s max par hold (au lieu de 10 s). Migration one-time dans `MilestoneService.ensureLoaded` (`career.milestones_migrated_0_5_0`) qui réécrit silencieusement les acquittements existants vers les nouveaux ids ; `UnlockKey.fromString` mappe les anciennes chaînes (`throat_hold_long → throatHold`, etc.) pour ne perdre aucun unlock. Les joueuses qui avaient `*Long` ont déjà la durée prouvée dans leurs capacités, donc aucune régression. Test `milestone_unlock_invariants_test` adapté : plus de seuil `mustReach`, juste un garde-fou contre la réintroduction des unlocks retirés.

### Corrigé
- **Supplier perdait les `milestoneId` de la session → niveau bloqué** — `SessionController.requestUpgrade` reconstruisait `_session` sans propager les 6 champs `milestoneId/milestoneStartTime/milestoneDurationSeconds` (+ `second*`). À `_finish`, `markIfPresent` recevait `null` → la body milestone jouée avant le Supplier n'était jamais `markCompleted`, et `sessionMilestoneUnlocks` restait vide → `canLevelUp` retournait `false` indéfiniment quand le pool n'était pas vide (cas joueur stefsub bloqué niveau 22 sur 28 sessions / 21 level-ups). Extraction de la logique de fusion en helper statique testable `SessionController.buildUpgradedSession` qui : (a) conserve les body milestones de l'ancienne session UNIQUEMENT si leur fenêtre `[start, start+dur]` est entièrement passée à l'instant du Supplier (sinon abandon — sinon `_finish` acquitterait une milestone non jouée) ; (b) ajoute les body milestones de `upcomingSession` décalées de `start + begDuration` (cas retry milestone) ; (c) prend le `finalMilestoneId` de `upcomingSession` (Supplier remplace le final, comme `finalStepTime`/`silentFinishStartTime`). Au passage : les champs `bpmEnd`, `chainAction`, `swallowMode`, `background` des steps de `upcomingSession` étaient perdus dans la copie — désormais propagés. Test de régression dédié (12 cas) dans `test/session_controller_supplier_test.dart`.
